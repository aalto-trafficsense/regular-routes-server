...
# mehrdad - new
#import urllib
#import requests    #moved to helpers 
#import urllib2     #moved to helpers
import polyline

from pyfiles.common_helpers import (get_distance_between_coordinates, point_distance, point_coordinates, http_request_with_get, OTPTimeStampToNormalDateTime, OTPDurationToNormalDuration)
from trip_planner import (Trip, TripPlanner)

LOG_DETAILS = True # generate extensive logs!

# -------- other helpers and definitions --------------------------
...
class TripMatchedWithPlannerResult:
    trip = None # its type should be PlannedTripLeg
    matchcount = 0
    bestmatchindex = 0
    error_code = 0
    error_msg = ''
...
#TODO change match planner code to dynamically get minSpeed vals by leg['mode']
...
class MassTransitMatchPlanner:
    def __init__(self):
        self.config1 = None

    def match_tripleg_with_publictransport(self, fromPlace, toPlace, trip_starttime, trip_endtime, all_recorded_trip_points):
        print "Input Trip Leg ...:"
        print "trip_starttime:", trip_starttime
        print "trip_endtime:", trip_endtime

        tripmatchres = TripMatchedWithPlannerResult()

        # TODO TODO, what about these params returned by otp journey planner? 
        ...
        # TODO IMPORTANT!!! Use TripPlanner.plan_a_trip() method instead *!!!!!!!!
        ...
        for itin in json_data['plan']['itineraries']: 
            duration = OTPDurationToNormalDuration(itin['duration'])        # duration of planned trip
            starttime = OTPTimeStampToNormalDateTime(itin['startTime'])    # start time of planned trip
            endtime = OTPTimeStampToNormalDateTime(itin['endTime'])         # ...
            ...
            # now look at legs of this itin to maybe find a match *
                ...
                for leg in planlegs:
                    ...
                    legdurationnormal = OTPDurationToNormalDuration(int(legduration)) # TODO why int() ?
                    legstarttime = OTPTimeStampToNormalDateTime(leg['startTime'])
                    legendtime = OTPTimeStampToNormalDateTime(leg['endTime'])
                        ...
                        if LOG_DETAILS:          
                            ...
                            for stop in intermstops: 
                                ...
                                print "stop[{3}][{4}]: {0}, @{1} .. @{2}".format(stopname, \
                                        OTPTimeStampToNormalDateTime(stop['arrival']), OTPTimeStampToNormalDateTime(stop['departure']), \
                    ...
                    if matchedbytime: #and matchedbyroute: # save this leg as a match * # TODO!!! check both!
                        ...
                        plannedmatch = PlannedTripLeg() # a new matched planned trip
