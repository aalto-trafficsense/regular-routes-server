...
"""
    Transforms
    ---------
    A bunch of functions for transforming back and forth between binary and multi-class representations.
    TODO: may need to work with sparse representations instead of binary
"""

def do_prune(count,p=0,p_max=2):
    ''' 
    pruning
    -----------------------------------------
    count: {01:50,10:203,11:2}
...
def transform_BR2PS(Y,p=0,p_max=0,n=0):
    ...
    -----------------------------------------
    Y: the label matrix
    p: the pruning to do (i.e., pruned sets) -- not yet implemented
    ...
    counts = count_up(Y)
    # 2. prune
    counts = do_prune(counts,p,p_max)
    # 3. map
